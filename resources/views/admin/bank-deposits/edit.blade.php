...
<div class="max-w-lg">
    <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
        ...
        <form action="{{ route('admin.bank-deposits.update', $bankDeposit) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select name="bank_account_id" required class="{{ $inp }} cursor-pointer">
                    ...
                    <option value="{{ $a->id }}" {{ old('bank_account_id', $bankDeposit->bank_account_id) == $a->id ? 'selected' : '' }}>{{ $a->bank_name }} — {{ $a->account_number }}</option>
                    @endforeach
                </select>
            </div>
            <div class="grid sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="date" name="deposit_date" value="{{ old('deposit_date', $bankDeposit->deposit_date->format('Y-m-d')) }}" required class="{{ $inp }}">
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-700 mb-1">Amount Deposited (৳) <span class="text-red-500">*</span></label>
                    <input type="number" name="amount" value="{{ old('amount', $bankDeposit->amount) }}" min="0.01" step="0.01" required class="{{ $inp }}">
                </div>
            </div>
            <div>
                <label class="block text-xs font-medium text-gray-700 mb-1">Source / Collection Reference</label>
                <input type="text" name="source_reference" value="{{ old('source_reference', $bankDeposit->source_reference) }}" class="{{ $inp }}">
            </div>
            <div>
                <label class="block text-xs font-medium text-gray-700 mb-1">Remarks</label>
                <textarea name="remarks" rows="2" class="{{ $inp }}">{{ old('remarks', $bankDeposit->remarks) }}</textarea>
            </div>
            <div>
                <label class="block text-xs font-medium text-gray-700 mb-1">Deposit Slip / Attachment</label>
                @if($bankDeposit->attachment_url)
                <p class="text-xs mb-1"><a href="{{ $bankDeposit->attachment_url }}" target="_blank" class="text-blue-600 hover:underline"><i class="fas fa-paperclip"></i> Current file</a> — upload to replace</p>
                @endif
            ...
            <div class="flex gap-3">
                <button type="submit" class="flex-1 bg-blue-600 text-white py-2.5 rounded-lg font-medium hover:bg-blue-700 transition-colors text-sm">Update Deposit</button>
