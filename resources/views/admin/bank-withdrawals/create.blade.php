@extends('layouts.app')
@section('title', 'Add Bank Withdrawal')
@section('page-title', 'Record Bank Withdrawal')
...
<div class="max-w-lg">
    <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
        ...
        @endif
        <form action="{{ route('admin.bank-withdrawals.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            @csrf
            ...
            <div>
                ...
                <select name="bank_account_id" required class="{{ $inp }} cursor-pointer">
                    ...
                    <option value="{{ $a->id }}" data-balance="{{ $a->available_balance }}" {{ old('bank_account_id', $selected) == $a->id ? 'selected' : '' }}>{{ $a->bank_name }} — {{ $a->account_number }} (Available ৳{{ number_format($a->available_balance, 2) }})</option>
                    @endforeach
                </select>
            </div>
            <div class="grid sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="date" name="withdrawal_date" value="{{ old('withdrawal_date', date('Y-m-d')) }}" required class="{{ $inp }}">
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="text" name="cheque_number" value="{{ old('cheque_number') }}" class="{{ $inp }}">
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-700 mb-1">Purpose</label>
                    <input type="text" name="purpose" value="{{ old('purpose') }}" placeholder="e.g. Event expense" class="{{ $inp }}">
                </div>
            </div>
            <div>
            ...
            <div class="flex gap-3">
                <button type="submit" class="flex-1 bg-blue-600 text-white py-2.5 rounded-lg font-medium hover:bg-blue-700 transition-colors text-sm">Save Withdrawal</button>
