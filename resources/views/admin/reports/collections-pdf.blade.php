...
<body>
    <h1>Unity Circle — Collections Report {{ $year }}{{ isset($month) && $month ? ' / ' . date('F', mktime(0,0,0,$month,1)) : '' }}</h1>
    <p class="subtitle">Generated: {{ now()->format('d M Y, g:i A') }} &nbsp;|&nbsp; Records: {{ $collections->count() }}</p>
    <table>
        <thead>
            <tr>
                <th>#</th>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ date('F', mktime(0,0,0,$c->month,1)) }} {{ $c->year }}</td>
                <td>৳{{ number_format($c->amount, 2) }}</td>
                <td>{{ ucfirst($c->payment_method) }}</td>
                <td>{{ $c->approved_at?->format('d M Y') }}</td>
            </tr>
            @endforeach
        </tbody>
        <tfoot>
